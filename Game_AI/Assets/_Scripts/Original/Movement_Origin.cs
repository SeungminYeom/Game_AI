...
public class Movement_Origin : MonoBehaviour
{
    ...
    // 추격하기
    protected Vector3 Pursuit(GameObject evader)
    {
        Vector3 toEvader = evader.transform.position - transform.position;

        double relativeHeading = Vector3.Dot(transform.forward, evader.transform.forward);

        if (Vector3.Dot(toEvader, transform.forward) > 0 && relativeHeading < 0.95)
        {
            return evader.transform.position - transform.position;
        }
        float lookAheadTime = toEvader.magnitude / 30;

        return (evader.transform.position + (evader.transform.forward * 10) * lookAheadTime) - transform.position;
    }

    // 도피하기
    protected Vector3 Evade(List<GameObject> pursuer)
    {
        ...
        if (pursuer.Count > 0)
        {
            ...
            {
                toPursuer += pursuer[i].transform.position + pursuer[i].transform.forward *
                    ((pursuer[i].transform.position - transform.position).magnitude / 30);
            }

            return toPursuer / pursuer.Count;
        }
